...
def _adapter_update_norms(model, snapshot):
    ...
    for name, before in snapshot.items():
        ...
        if name.startswith('native_axis_adapter.depth_gate.'):
            gate_sq += update_sq
            ...
            core_sq += update_sq
    return core_sq ** 0.5, alpha_sq ** 0.5, alpha_depth_sq ** 0.5, gate_sq ** 0.5
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if loss_scaler is None:
            ...
            if (data_iter_step + 1) % update_freq == 0 and adapter_step_snapshot is not None:
                core_update_norm, alpha_update_norm, alpha_depth_update_norm, depth_gate_update_norm = _adapter_update_norms(
                ...
                adapter_diagnostics['depth_gate_update_norm'] = depth_gate_update_norm
                adapter_step_snapshot = None
            if (data_iter_step + 1) % update_freq == 0:
                optimizer.zero_grad()
                if model_ema is not None:
